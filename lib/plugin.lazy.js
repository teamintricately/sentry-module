...
async function loadSentry (ctx, inject) {
  ...
  /* eslint-disable object-curly-spacing, quote-props, quotes, key-spacing, comma-spacing */
  let config = <%= serialize(options.config) %>

  const publicRuntimeConfigKey = <%= serialize(options.publicRuntimeConfigKey) %>
  if (ctx.$config && publicRuntimeConfigKey && ctx.$config[publicRuntimeConfigKey]) {
    const deepMerge = await import(/* <%= magicComments.join(', ') %> */ 'deepmerge')
    config = deepMerge(config, ctx.$config[publicRuntimeConfigKey])
  }
