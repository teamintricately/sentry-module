...
export async function buildHook (moduleContainer, options, logger) {
  ...
  moduleContainer.addPlugin({
    ...
    options: {
      ...
      publicRuntimeConfigKey: options.publicRuntimeConfigKey,
      config: {
        dsn: options.dsn,
...
export async function initializeServerSentry (moduleContainer, options) {
  ...
  const { publicRuntimeConfigKey } = options

  if (publicRuntimeConfig && publicRuntimeConfigKey && publicRuntimeConfig[publicRuntimeConfigKey]) {
    options.serverConfig = deepMerge(options.serverConfig, publicRuntimeConfig[publicRuntimeConfigKey])
  }
